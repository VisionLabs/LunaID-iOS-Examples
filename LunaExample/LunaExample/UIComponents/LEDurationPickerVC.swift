...
typealias DurationChangedHandler = (_ newTime: CGFloat) -> Void

class LEDurationPickerVC: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    var periodChangedHandler: DurationChangedHandler?
    private var initialValue: CGFloat
    private let limit: CGFloat
    private let limitPower: CGFloat
    
    private let titleLabel = UILabel(frame: .zero)
    
    init(title: String, limit: CGFloat, limitPower: CGFloat, initialValue: CGFloat) {
        self.initialValue = initialValue
        self.limit = limit
        self.limitPower = limitPower
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        if limitPower > 10 {
            return String(format: "%.2f", powerValue(CGFloat(row)))
        }
        
        return String(format: "%.1f", powerValue(CGFloat(row)))
    }
    
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        periodChangedHandler?(powerValue(CGFloat(row)))
    }
    
    private func powerValue(_ value: CGFloat) -> CGFloat {
        return CGFloat(value / limitPower)
    }
}
