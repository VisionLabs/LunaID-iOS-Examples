...
namespace fsdk {
	...
	struct IFaceLandmarksDetector : IRefCounted {
		 ...
		 * @see Ref, Span, Image, Detection, Landmarks5, IFaceLandmarksBatch, ResultValue and FSDKError for details.
		 ...
		 * */
		virtual ResultValue<FSDKError, Ref<IFaceLandmarksBatch>> detectLandmarks5(
			Span<const Image> images,
			Span<Span<const Detection>> detections) const noexcept = 0;
		 ...
		 * @see Ref, Span, Image, Detection, Landmarks68, IFaceLandmarksBatch, ResultValue and FSDKError for details.
		 * @note images format must be R8G8B8, @see Format.
		 * @note all spans should be based on user owned continuous collections.
		 * @note all spans should be equal size.
		 * */
		virtual ResultValue<FSDKError, Ref<IFaceLandmarksBatch>> detectLandmarks68(
			Span<const Image> images,
			Span<Span<const Detection>> detections) const noexcept = 0;
		 ...
		 * @see Ref, Span, Image, Detection, Landmarks5, IFaceDetectionBatch, ResultValue and FSDKError for details.
		 * @note images format must be R8G8B8, @see Format.
		 * @note all spans should be based on user owned continuous collections.
		 * @note all spans should be equal size.
		 * */
