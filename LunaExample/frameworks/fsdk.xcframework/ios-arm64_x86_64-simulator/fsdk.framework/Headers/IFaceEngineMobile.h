...
#include <fsdk/Estimators/INIRLivenessEstimator.h>
...
namespace fsdk {
	...
	struct IFaceEngineMobile : IRefCounted {
		 ...
		 * */
		virtual ResultValue<FSDKError, IBestShotQualityEstimatorPtr>
		 ...
		 * */
		virtual ResultValue<FSDKError, ILivenessOneShotRGBEstimatorPtr>
		 ...
		 * */
		virtual ResultValue<FSDKError, ILivenessDepthRGBEstimatorPtr>
		createLivenessDepthRGBEstimator() noexcept = 0;

		/**
		 * @brief Creates NIRLiveness estimator.
		 * @param [in] mode is the option to set estimation scenario. @see NIRLivenessMode.
		 * @param [in] launchOptions launch options for fine grained control over target device resources.
		 * @return ResultValue with error code and INIRLivenessEstimatorPtr instance.
		 * @see ResultValue, FSDKError, INIRLivenessEstimatorPtr, LaunchOptions
		 * */
		virtual ResultValue<FSDKError, INIRLivenessEstimatorPtr>
		createNIRLivenessEstimator(const NIRLivenessMode mode = NIRLivenessMode::Default) noexcept = 0;
		 ...
		 * @returns Error code; @see FSDKError and Descriptor if succeed, null if failed.
		 * */
		virtual ResultValue<FSDKError, IDescriptorPtr>
		 ...
		 * descriptor batch you must pass version, minimum version of human descriptor is
		 ...
		 * */
		virtual ResultValue<FSDKError, IDescriptorBatchPtr>
		 ...
		 * descriptor you must pass version, minimum version of human descriptor is
		 * DV_MIN_HUMAN_DESCRIPTOR_VERSION == 102
		 * @see DescriptorVersion
		 * @returns Error code; @see FSDKError and Descriptor extractor if succeed, null if failed.
		 * */
		virtual ResultValue<FSDKError, IDescriptorExtractorPtr>
	 ...
	 * @param [in] dataPath [optional] path to folder with FSDK data. Default: ./data (on windows),
