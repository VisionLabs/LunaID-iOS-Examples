...
class AuthOCRViewController: UIViewController, UITextFieldDelegate, LunaIdentifyFlowDelegate, LunaVerifyFlowDelegate {
    ...
    private let loginButtonWithDocuments = LCRoundButton(type: .custom)
    private let usernameField = TextField(frame: .zero)
    
    private var currentFlow: LunaOCRFlowProtocol?
    private let configuration: LCLunaConfiguration = LCLunaConfiguration.defaultConfig()
    private let legacyStoryboard = UIStoryboard(name: "Main", bundle: Bundle(for: AuthOCRViewController.self))
    ...
    private func validateUserName(_ textField: TextField) {
        guard let text = textField.text else {
            ...
            loginButtonWithDocuments.isEnabled = true
            loginButton.setTitle("auth.identify".localized(), for: .normal)
            loginButtonWithDocuments.setTitle("auth.identify_with_ocr".localized(), for: .normal)
            return
        }
        ...
        if text.isEmpty {
            ...
            do {
                ...
                loginButtonWithDocuments.isEnabled = isValid
                loginButton.setTitle("auth.verify".localized(), for: .normal)
                loginButtonWithDocuments.setTitle("auth.verify_with_ocr".localized(), for: .normal)
            } catch {
                textField.isValid = false
                loginButton.isEnabled = false
                loginButtonWithDocuments.isEnabled = false
                loginButton.setTitle("auth.verify".localized(), for: .normal)
                loginButtonWithDocuments.setTitle("auth.verify_with_ocr".localized(), for: .normal)
            }
        }
    ...
    private func login(_ sender: UIButton) {
        loginScenario(withOCR: false)
    }
    
    @objc
    private func loginWithDocuments(_ sender: UIButton) {
        loginScenario(withOCR: true)
    }
    ...
    private func verify(faceID: String) {
        guard let newFlow = LunaOCRFlowBuilder.verify(faceID: faceID,
                                                      configuration: configuration,
                                                      delegate: self) as? LunaOCRFlowProtocol else {
            fatalError("Mess with types")
        }
        currentFlow = newFlow
    ...
    private func identify() {
        guard let newFlow = LunaOCRFlowBuilder.identify(configuration: configuration,
                                                        delegate: self) as? LunaOCRFlowProtocol else {
            fatalError("Mess with types")
        }
        currentFlow = newFlow
    ...
    private func createLayout() {
        ...
        loginButtonWithDocuments.translatesAutoresizingMaskIntoConstraints = false
        loginButtonWithDocuments.setTitle("Identification with documents", for: .normal)
        loginButtonWithDocuments.addTarget(self, action: #selector(loginWithDocuments), for: .touchUpInside)
        view.addSubview(loginButtonWithDocuments)
        ...
        NSLayoutConstraint.activate([
            ...
            loginButtonWithDocuments.topAnchor.constraint(equalTo: loginButton.bottomAnchor, constant: LoginButtonsBetween),
            loginButtonWithDocuments.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: CommonSideOffset),
            loginButtonWithDocuments.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -CommonSideOffset),
            loginButtonWithDocuments.heightAnchor.constraint(equalToConstant: LoginButtonsHeight),
    ...
    private func loginScenario(withOCR: Bool) {
        configuration.ocrEnabled = withOCR
